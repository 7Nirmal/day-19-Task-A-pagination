...
let currentpage = 1;
let rows = 10;
let totalpages=Math.ceil(rawdata.length/rows);
//creating function to filter amount of data
function displaydata(page,rows){
 let start = (currentpage-1) * rows;
 let end = start + rows;
 let data = rawdata.slice(start,end);
 // calling table function which forms rows with filtered data
 buildtable(data);
}
